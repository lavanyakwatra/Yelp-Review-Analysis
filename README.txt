...
5. Notebook 3 - Preprocessing & Modelling
...
This notebook begins with defining the problem space & reading in all of the available data and taking a sample of it for exploration in the next notebook. This notebook was hosted on my local computer as the files were too large & required AWS support to use a larger instance. This notebook takes time to run and will require downloading the relevant datasets from the S3 bucket or Yelp's website.
...
This notebook uses a smaller sample of 10,000 reviews, with corresponding user & business data points, to show exploratory analysis, final preprocessing including text vectorization & feature engineering to prepare the dataset for modelling. A few parameter optimization techniques are implemented to evaluate the accuracy scores of classification. This notebook was on a Sagemaker AWS Instance where the sample data frame is stored. 

C. Yelp Review Analysis Report:

A business report on my Yelp Review Analysis & further steps.
